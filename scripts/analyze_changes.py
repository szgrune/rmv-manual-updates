...
def _inject_chapter_and_images(change: dict, new_sections: list[dict]) -> None:
    ...
    change.setdefault("chapter", "Other Updates")
...
def analyze_pair(
    ...
    if not force and database.get_change_analysis(old_year, new_year) is not None:
        print(f"  [{old_year}→{new_year}] Already analyzed — skipping (use --force to redo)")
        return

    print(f"\n{'='*60}")
    print(f"Analyzing {old_year} → {new_year}")
    print(f"{'='*60}")
    ...
    if not old_sections or not new_sections:
        print(f"  ERROR: Missing sections. Run extract_pdfs.py first.")
        return
    ...
    # ── Split 2026 into chunks and compare each against full old manual ────────
    new_chunks = _split_into_chunks(new_full_text, CHUNK_SIZE)
    print(f"  2026 split into {len(new_chunks)} chunks")

    all_changes: list[dict] = []
    chapter_overviews: list[str] = []
    ...
    for i, chunk in enumerate(new_chunks, 1):
        ...
        overview = result.get("chapter_overview", "").strip()
        if overview and "no significant" not in overview.lower():
            chapter_overviews.append(overview)
    ...
    # Filter out topics already reported as a change
    ...
    if candidates:
        ...
        time.sleep(SLEEP_SECS)
        removed = analyzer.find_removed_content(candidates, new_full_text, old_year)
        for item in removed:
            change = {
    ...
    # ── Overview ──────────────────────────────────────────────────────────────
    print(f"\n  Generating overview ({len(all_changes)} total changes)...", end=" ", flush=True)
    overview_text = analyzer.generate_overview(all_changes, old_year)
    print("done")
    ...
    database.upsert_change_analysis(
        ...
        sections=all_changes,
    )
    print(f"\n✓ [{old_year}→{new_year}] Complete — {len(all_changes)} changes written to DB")
...
def main():
    ...
    for old_year in prior_years:
        analyze_pair(old_year, latest, analyzer, force=force)
